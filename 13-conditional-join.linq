...
/* So why did I spent so much time showing how to do the different kinds of joins
   ...
   The problem is, in the real world the requirements are not as "synthetic" as in
   the previous examples. No requirements engineer will come to you and request a
   plain inner join of `Foo`s and `Bar`s. It's more likely that he or she says 
   you should "marry" every second `Foo` with all the `Bar`s that "belong to it".
